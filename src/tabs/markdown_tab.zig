...
//! on the caret's line. The Preview / Source switch in the header, ⌘E, the
//! View menu and the palette all flip between the two.
...
// The Preview / Source switch in the card header.
...
pub const MarkdownTab = struct {
    ...
    pub fn draw(self: *MarkdownTab, ui: *Ui, rect: Rect, focused: bool) void {
        ...
        const sw = self.switchWidth(ui);
        const body = self.file.frameWith(ui, rect, &meta_buf, sw) orelse return;
        self.drawSwitch(ui, viewer.trailingRect(viewer.card(rect), sw, switch_h));
